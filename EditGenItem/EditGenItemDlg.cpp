...
BOOL CEditGenItemDlg::OnInitDialog()
{
	...
	char* szColumnName1[nColumnCount] = { "掉落數量", "機率" };
	...
	char* szColumnNames1[nColumnCounts] = { "Gen ID", "機率" };
	...
	char* szColumnNameid1[nColumnCountid] = { "主 ID", "子 ID", "機率" };
	...
	// 設定這個對話框的圖示。框架若主視窗非對話框會自動設定。
	SetIcon(m_hIcon, TRUE);			// 設定大圖示
	SetIcon(m_hIcon, FALSE);		// 設定小圖示

	return TRUE;  // 除非將焦點設到控制項，否則請回傳 TRUE
}
...
// 若加入最小化按鈕，以下程式將負責繪製圖示。
// 文件/檢視架構下會由框架自動完成。
void CEditGenItemDlg::OnPaint()
{
	if (IsIconic())
	{
		CPaintDC dc(this); // 繪圖設備物件
...
*/
	...
	CFileDialog dlg(FALSE, ".crowsale", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter, (CEditGenItemDlg*)this);
...
BOOL CEditGenItemDlg::UpdateData(const char* szFileName)
{
	...
	*/
	...
	if (!StrFile.Open(szFileName, TRUE)) // 開啟並解碼字串檔案類別
		return FALSE;

	while (StrFile.GetNextLine(strLine)) // 一行一行讀檔
	{
		strLine += "\r\n";
		strTemp += strLine;
	}

	strTemp.SetAt(strTemp.GetLength() - 2, NULL); // 移除最後 \r\n
...
BOOL CEditGenItemDlg::SaveData(const char* lpszPathName)
{
	...
	CFile file;
	file.Open(lpszPathName, CFile::modeCreate | CFile::modeWrite);	// 開啟檔案
	file.Write(&nVersion, sizeof(int));							// 寫入版本號
	file.Write(szBuffer, nLen);		// 寫入檔案
	file.Close();
...
void CEditGenItemDlg::OnBnClickedButtonUpdateGenItem()
{
	...
	CString oneline, tryall;
	AppendAllTexts.Format("/////////////////////////////////////////////////////\r\n//由 EditGenitem (c)RSiLENT 產生 GENITEM\r\t\t\r\n//////////////////////////////////////////////////////");
	m_fGenRate = GetWin_Num_float(this, IDC_EDIT_GEN_RATE);
	oneline.Format("GENRATE		%.2f", m_fGenRate);
